...
First we sought to develope a single model that would differentiate normal individuals from those with any type of colonic lesion. We determined the optimal model using the AUC-RF algorithm for maximizing the area under the curve (AUC) of the RF model [@calle2011auc]. The optimal model combining FIT and the microbiota used 23 OTUs. Of those OTUs, 14 were members of the Clostridia, including 10 from the Lachnospiraceae family (OTUs 14, 44, 8, 88, 60, 22, 9, 13, 87, 31) and 2 from the Ruminococcaceae family (OTU29, OTU11). Three OTUs were associated with the genus *Bacteroides* (OTUs 3, 7, 2). The remaining OTUs were associated with *Porphyromonas* (OTU105), *Parabacteroides* (OTU49), *Streptococcus* (OTU20), and Enterobacteriaceae (OTU28). Interestingly the majority of OTUs used in the model were enriched in normal patients. 

The model had an AUC of `r round(canc_roc$auc, 3)` for comparing cancer to normal, `r round(ade_roc$auc, 3)` for adenoma to normal, and `r round(les_roc$auc, 3)` for all lesions to normal (Figure 2?). We defined an optimal cutoff for the RF model based on Youden's J statistic (Youden, 1950). At this cutoff, the RF model had a sensitivity of `r canc_mod_sens` for cancers. The model's sensitivity was higher for later stage cancers with sensitivities of `r 100*signif(s1_sens, 3)`, `r 100*signif(s2_sens, 3)`, `r 100*signif(s3_sens, 3)`, and `r 100*signif(s4_sens, 3)` for stages I, II, III, and IV, respectively (Figure 3). The RF model had similar sensitivity for advanced (`r 100*signif(advAde_sens, 3)`) and nonadvanced (`r 100*signif(nonAdv_sens, 3)`) adenomas.
...
The accuracy of the RF model would likely be improved by incorporating additional data. For example several bacterial toxins are capable of exacerbating tumorigeneisis, and there is some evidence that methanogenic archaea are associated with CRC [citation]. These microbial biomarkers could be combined with 16S rRNA sequencing for a more comprehensive analysis of the microbiome. Furthermore host-associated biomakers, like _KRAS_ mutations, _B_-actin, or aberrant methylation of _NDRG4_ and _BMP3_ could add to the our model [citation]. We have shown previously that patient demographic information can strengthen microbiome-based models for distinguishing healthy patients from those with CRC [@zackular2014human]. Thus interpreting both host and microbial biomarkers in the context of an individual's risk could further improve screening accuracy. 
