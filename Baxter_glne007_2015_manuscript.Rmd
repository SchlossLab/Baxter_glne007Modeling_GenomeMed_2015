...
```{r microbiota_cancer, warning=F, results='hide', echo=F, fig.width=7, fig.height=7, cache=T}
#generate cancer vs normal model
...
plot(canc_res, log='x', ylab='Microbiota Model Result', xlab='FIT Result', xaxt='n')
...
```{r microbiota_adenoma, warning=F, results='hide', echo=F, fig.width=7, fig.height=7, cache=T}
...
plot(ade_res, log='x', ylab='Microbiota Model Result', xlab='FIT Result', xaxt='n')
...
```{r otu_stripcharts, warning=F, echo=F, fig.width=7.5, fig.height=10, results='hide'}
#Importance Plot
...
canc_abunds <- all_data[all_data$dx!='normal', canc_otus]/10000 + 1e-4
...
axis(2, at=seq(1,index-2,2), labels=tax[canc_otus, 'Label'], las=1, tick=F, cex.axis=1)
axis(1, at=c(1e-4, 1e-3, 1e-2, 1e-1, 1), label=c("0", "0.1", "1", "10", "100"))
legend('right', legend=c("Cancer", "Normal"), pch=c(21, 21), pt.bg=c("red","grey80"))
...
png('results/figures/Schloss_EDfig1.png', height=6, width=6, units='in', res=150)
plot(les_model)
dev.off()
...
png(file='results/figures/fig1A_ROCcurve.png', height=6, width=6, units='in', res=150)
plot(c(1,0),c(0,1), type='l', lty=2, xlim=c(1.01,0), ylim=c(-0.01,1.01), xaxs='i', yaxs='i', ylab='', xlab='')
...
axis(1, at=c(0.8,2,3.2), labels=c('Normal','Adenoma','Cancer'))
axis(2, at=c(0.1,1.1,10.1,100.1,1000.1), labels=c(0,1,10,100,1000))
mtext(side=2, text="FIT Result (ng/ml)", line=2)
abline(h=100, lty=2)

plot(1, type="n", xlim=c(0.5,3.5), ylim=c(0,1), ylab="", xlab="", xaxt="n")
...
mtext(side=2, text="MMT Probability of Lesion", line=2)
axis(1, at=c(0.8,2,3.2), labels=c('Normal','Adenoma','Cancer'))
