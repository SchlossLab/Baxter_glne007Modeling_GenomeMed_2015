...
legend('bottomright', legend=c(sprintf('MMT-Les. vs Norm.: %.3g',les_roc$auc),
                               ...
                               sprintf('FIT-Ade. vs Norm.: %.3g',ade_fit_roc$auc)),lty=c(1,2,1,2,1,2), lwd=2, col=c('darkred','darkred','red','red','orange','orange'), bty='n', cex=0.8)
...
```{r figS3_mmtModel, warning=F, echo=F, fig.width=7.5, fig.height=10, results='hide'}
...
points(coords(m_roc, x=cutoff, input='threshold', ret='specificity'),coords(m_roc, x=cutoff, input='threshold', ret='sensitivity'), col='orange', cex=1.5, pch=16)
...
legend('bottomright', legend=c(sprintf('All Subjects: %.3g',les_roc$auc),
                               ...
                               ),lty=c(1,1,1,0), lwd=2, col=c('purple','orange','red'), bty='n')
...
**Figure 2. Comparing MMT to FIT.** (A) ROC Curves for the MMT (solid lines) and FIT (dashed lines) for distinguishing normal from any lesion (purple), normal from cancer (red) and normal from adenoma (orange). Filled dots show the sensitivity and specificity of the MMT at the optimal cutoff (0.622).  Open dots show the sensitivity and specificity of FIT at the 100ng/ml cutoff. (B) Strip charts showing the results for FIT and the MMT. Dashed lines show the cutoff for each test. Points with a FIT result of 0 are jittered to improve visibility.
