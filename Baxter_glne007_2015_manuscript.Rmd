...
colnames(stats_table) <- c('Most Advanced Finding','','RF Model','','FIT','')
...
_**Statistical Methods.**_ All statistical analyses were performed using R. Random Forest models were generated using the AUCRF package [@calle2011auc]. The AUC of ROC curves was compared using the method described by DeLong et al. [@delong1988comparing]. The optimal cutoff for the RF model was determined using Youden's _J_ statistic as implemented in the pROC package in R [@youden1950index]. The sensitivities of FIT and the RF model were compared using McNemar's chi-squared test.
...
**Figure 1.** (A) ROC Curves for the RF model (solid lines) and FIT (dashed lines) for distinguishing normal from any lesion (purple), normal from cancer (red) and normal from adenoma (blue). Filled dots show the sensitivity and specificity of the RF model at the optimal cutoff (0.622).  Open dots show the sensitivity and specificity of FIT at the 100ng/ml cutoff. (B) Strip charts showing the results for FIT and the RF model. Dashed lines show the cutoff for each test. Points with a FIT result of 0 are jittered to improve visibility.


![](results/tables/Schloss_table1.png|width=50)  
**Table 1.** Table of sensitivities and specificities for the RF model and FIT. The 95% confidence intervals were computed with 2000 stratified bootstrap replicates

![](results/figures/Schloss_fig2.png)  
**Figure 2.** Barplot of sensitivities for the RF model and FIT for each stage of tumor development.  P-values based on McNemar's chi-squared test.

![](results/figures/Schloss_fig3.png)  
**Figure 3.** Scatter plot of the results of the RF model and FIT for each sample. Dashed lines show the cutoff for each test. 
...
**Extended Data Figure 2.** Stripchart of the relative abundances of each OTU in the RF model.

![](results/figures/Schloss_EDfig3.png)  
**Extended Data Figure 3.** Stripchart of RF model results for each sample based on FIT result.

![](results/tables/Schloss_EDtable1.png)  
**Extended Data Table 1.** Positive and negative predictive values for the RF model and FIT based on published estimates of CRC prevalence.

![](results/figures/Schloss_EDfig4.png)  
**Extended Data Figure 4.** ROC curves of RF model separated by gender (left). Stripchart of RF model results by gender (right). 
