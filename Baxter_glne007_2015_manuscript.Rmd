...
csl: science-translational-medicine.csl
output:
  word_document:
...
points(coords(otu_canc_roc, x='best', ret='spec'),coords(otu_canc_roc, x='best', ret='sens'), pch=16, cex=1.5)
...
points(canc_res[canc_res$fit>100 | canc_res$micro>0.459,], pch=21, bg='grey')
points(canc_res[canc_res$fit>100 & canc_res$micro>0.459,], pch=21, bg='black', col='white', lwd=0.7)
...
abline(h=0.459, lty=3)
abline(v=100, lty=3)
mtext('B', at=0.04, font=2, side=3)

canc_pos <- cbind(all_data[all_data$dx=='cancer','fit_result']>100, otu_canc_probs[otu_canc_data$lesion==1,2]>0.459)
...
otu_ade_roc <- roc(ade_data$lesion~otu_ade_probs[,2])
ade_fit_roc <- roc(otu_ade_data$lesion~les_data$fit_result[meta$dx!='cancer'])
...
points(coords(otu_ade_roc, x=0.702, input='thr', ret='spec'),coords(otu_ade_roc, x=0.702, input='thr', ret='sens'), pch=16, cex=1.5)
...
points(ade_res[ade_res$fit>100 | ade_res$micro>0.702,], pch=21, bg='grey')
points(ade_res[ade_res$fit>100 & ade_res$micro>0.702,], pch=21, bg='black', col='white', lwd=0.7)
axis(1, labels=c(0,1,10,100,1000), at=c(0.1,1.1,10,100,1000))
mtext(side=2, text='Microbiota Model Result', line=2.5)
mtext(side=1, text='FIT Result', line=2.5)
abline(h=0.702, lty=3)
abline(v=100, lty=3)
...
pdf('results/figures/supp_fig1.pdf', height=6, width=8)
...
png('results/figures/Schloss_EDfig2.png', width=7, height=9, units='in', res=150)
...
png(file='results/figures/Schloss_EDfig3.png', height=7, width=7, units='in', res=150)
...
png('results/figures/Schloss_fig3.png', width=7, height=6, units='in', res=150)
...
png('results/figures/Schloss_fig2.png', height=6, width=9, units='in', res=150)
par(mar=c(4,4,2,2))
...
#render('results/tables/table1.md', output_format='png_document', output_file='table1.png')
...
write(kable(pp_table), file='results/tables/supp_table1.md')
#render('results/tables/supp_table1.md', output_format='pdf_document', output_file='Schloss_EDtable1.pdf')
...
png('results/figures/Schloss_EDfig4.png', height=6, width=9, units='in', res=150)
...
_???_  
We characterized the bacterial communities of stool samples from 490 patients using 16S rRNA gene sequencing.  Among these patients, 120 had CRC, 198 had adenomas, and 172 had no colonic lesions. First we tested whether a random forest model could distinguish healthy individuals from those with cancer. We determined the optimal model using the AUC-RF algorithm for maximizing the area under the curve (AUC) of the receiver operating characteristic (ROC) curve for the random forest model [@calle2011auc]. The optimal model utilized 22 bacterial populations (`r canc_varImp`). Consistent with previous observations, the bacteria most strongly associated with CRC belonged to taxa more commonly associated with the oral cavity, including Pophyromonas, Fusobacterium, Parvimonas, Peptostreptococcus,and Gemella [warren2013co,zeller2014potential] (`r canc_stripChart`). The ROC curve for the model had an AUC of 0.837, which is similar to AUCs reported for other microbiota-based models for CRC, but lower than that of FIT (`r signif(canc_fit_roc$auc, 3)`). This confirms that random forest model based on bacterial abundances can distinuish healhty individuals from those with cancer.

We also generated a model for differentiating healthy indviduals from those with any type of lesion (i.e. adenoma or carcinoma).  

  

_Generating the MMT._
In addition to characterizing the bacterial community, we tested each sample for the concentration of hemoglobin using FIT. With these data we sought to determine whether incorporating FIT and microbiota data into a single model would the sensitivity for detecting colonic lesions. Therefore we developed a random forest model that incorporated both the microbiota and FIT data and would differentiate normal individuals from those with any type of colonic lesion (i.e. adenoma or carcinoma). The optimal model combining hemoglobin results and the microbiota used 23 OTUs (`r aucrf_curve`). Of those OTUs, 16 were members of the Firmicutes phylum, including 3 from the Ruminococcaceae family and 10 from the Lachnospiraceae family, the predominant producers of butyrate in the gut [@pryde2002microbiology] (`r otu_table`). Three OTUs were associated with the genus *Bacteroides*. The remaining OTUs were associated with *Porphyromonas*, *Parabacteroides*, *Collinsella*, and Enterobacteriaceae. The OTU associated with *Porphyromonas* was most closely related to *Porphyromonas asaccharolytica*, which has been previously shown to be predictive of CRC [@rex2009american;@zackular2014human]. Like other studies[@kostic2012genomic;@zackular2014human] we also observed an OTU associated with *Fusobacterium nucleatum* that was enriched in cancer samples, however its relative abundance did not add sufficient information to be included in the model. Interestingly the majority of OTUs used in the model, especially the Lachnospiraceae, were enriched in normal patients, suggesting that a loss of beneficial organisms in addition to the emergence of pathogens may be indicative of CRC development.
...
![](results/figures/Schloss_fig1.png)  
**Fig. 1.** Comparing MMT to FIT. (A) ROC Curves for the MMT (solid lines) and FIT (dashed lines) for distinguishing normal from any lesion (purple), normal from cancer (red) and normal from adenoma (blue). Filled dots show the sensitivity and specificity of the MMT at the optimal cutoff (0.622).  Open dots show the sensitivity and specificity of FIT at the 100ng/ml cutoff. (B) Strip charts showing the results for FIT and the MMT. Dashed lines show the cutoff for each test. Points with a FIT result of 0 are jittered to improve visibility.

![](results/figures/Schloss_fig2.png)  
**Fig. 2.** Sensitivities for FIT and MMT for each stage of tumor development.  P-values based on McNemar's chi-squared test.

![](results/figures/Schloss_fig3.png)  
**Fig. 3.** Results of FIT and MMT for each sample. Dashed lines show the cutoff for each test. Points with a FIT result of 0 are jittered to improve visibility.

###Supplementary Figures

![](results/figures/Schloss_EDfig1.png)  
**Supplementary Figure 1.** Variable selection for MMT. Change in out-of-bag AUC with number of features in the MMT. The optimal model contains 24 features and has an AUC of 0.829.

![](results/figures/Schloss_EDfig2.png)  
**Supplementary Figure 2.** Bacterial OTUs in MMT. Stripchart of the relative abundances of each OTU in the MMT with black lines at the medians.

![](results/figures/Schloss_EDfig3.png)  
**Supplementary Figure 3.**  MMT Probabilities based on FIT results. Stripchart of MMT results for samples with a positive or negative FIT result.

![](results/figures/Schloss_EDfig4.png)  
...
![](results/tables/Schloss_EDtable1.png)  
**Supplementary Table 1.** Estimated MMT performance on average risk population. Number and proportion of true positives identified through FIT and MMT in the United States in adults 50-75 years of age, based on published estimates of CRC prevalence. Far right column shows percentage of true positives identified among individuals with a negative FIT result.
