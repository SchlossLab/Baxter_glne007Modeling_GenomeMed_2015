...
```{r otu_stripchart, echo=F, results='hide'}
otus <- c('Otu000057','Otu000105','Otu000260','Otu000264','Otu000281','Otu000310','Otu000356')

norm_abunds <- all_data[all_data$dx=='normal', otus]/10000 + 1e-4
canc_abunds <- all_data[all_data$dx=='cancer', otus]/10000 + 1e-4
ade_abunds <- all_data[all_data$dx=='adenoma', otus]/10000 + 1e-4

tax_file <- read.delim('data/glne007.0.03.trim.tax', sep='\t', header=T, row.names=1)
tax <- paste(tax_file[otus, 'Classification'], '\n(', tax_file[otus, 'OTU'], ')', sep='')

pdf('results/figures/fig?_otuStripchart.pdf', height=8, width=6)
par(mar=c(5, 9, 0.5, 0.5))
plot(1, type="n", ylim=c(0,length(otus)*3-1), xlim=c(1e-4,1), log="x", ylab="", xlab="Relative Abundance (%)", xaxt="n", yaxt="n")
index <- 1
for(x in otus){
  stripchart(at=index-0.7, jitter(norm_abunds[,x], amount=1e-5), pch=21, bg="grey", method="jitter", jitter=0.2, add=T, cex=1)
  stripchart(at=index, jitter(ade_abunds[,x], amount=1e-5), pch=21, bg="blue", method="jitter", jitter=0.2, add=T, cex=1)
  stripchart(at=index+0.7, jitter(canc_abunds[,x], amount=1e-5), pch=21, bg="red", method="jitter", jitter=0.2, add=T, cex=1)
  segments(mean(norm_abunds[,x]),index-1,mean(norm_abunds[,x]),index-0.4, lwd=3)
  segments(mean(ade_abunds[,x]),index-0.3 ,mean(ade_abunds[,x]),index+0.3, lwd=3)
  segments(mean(canc_abunds[,x]),index+1,mean(canc_abunds[,x]),index+0.4, lwd=3)
  index <- index + 3
}

axis(2, at=seq(1,index-2,3), labels=c(tax), las=1, tick=F, cex.axis=1)
axis(1, at=c(1e-4, 1e-3, 1e-2, 1e-1, 1), label=c("0", "0.1", "1", "10", "100"))
legend('topright', legend=c('Cancer','Adenoma', 'Normal'), pch=21, pt.bg=c('red','blue','grey'))
dev.off()
...
pdf(file='results/figures/fig?_ROCcurve.pdf', height=6, width=6)
...
pdf('results/figures/fig?_modelOTUstripchart.pdf', width=7, height=12)
...
index <- 1
for(i in les_otus){
  stripchart(at=index-0.35, jitter(norm_abunds[,i], amount=1e-5), pch=21, bg="grey", method="jitter", jitter=0.2, add=T, cex=1)
  stripchart(at=index+0.35, jitter(les_abunds[,i], amount=1e-5), pch=21, bg="green4", method="jitter", jitter=0.2, add=T, cex=1)
...
axis(1, at=c(1e-4, 1e-3, 1e-2, 1e-1, 1), label=c("0", "0.1", "1", "10", "100"))
legend('topright', legend=c("Lesion", "Normal"), pch=c(21, 21), pt.bg=c("green4","grey"))
dev.off()
```

```{r model_stripCharts, warning=F, echo=F, fig.width=8, fig.height=6, results='hide'}
pdf(file='results/figures/fig?_stripCharts.pdf', height=7, width=9)
...
pdf(file='results/figures/suppfig_negFITstripchart.pdf', height=7, width=7) 
...
pdf('results/figures/fig?_FITvsRF.pdf', width=7, height=6)
...
pdf('results/figures/fig?_sensitivityBarplot.pdf', height=6, width=9)
par(mar=c(4,4,2,2))
...
write(kable(stats_table), file='results/tables/table.md')

#render('results/tables/table.md', output_format='pdf_document', output_file='table.pdf')
...
pdf('supp_maleFemale.pdf', height=6, width=9)
...
otu_table <- 'Extended Data Table 1'
pred_values <- 'Extended Data Table 2'
fit_neg_strip <- 'Extended Data Fig. 2'
...
### Methods Summary
FIT was performed using OC FIT-CHEK sampling bottles and processed using an OC-Auto Micro 80 automated system (Polymedco Inc.). The V4 region of the bacterial 16S rRNA gene was amplified using custom barcoded primers and sequenced as described previously using an Illumina MiSeq sequencer [@miseq].
