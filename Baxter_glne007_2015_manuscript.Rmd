...
_???_  
We characterized the bacterial communities of stool samples from 490 patients using 16S rRNA gene sequencing.  Among these patients, 120 had CRC, 109 had advanced adenomas, 89 had non-advanced adenomas, and 172 had no colonic lesions. First we tested whether a random forest model could distinguish healthy individuals from those with CRC. We determined the optimal model using the AUC-RF algorithm for maximizing the area under the curve (AUC) of the receiver operating characteristic (ROC) curve for the random forest model [@calle2011auc]. The optimal model utilized 22 bacterial populations (`r canc_varImp`). Consistent with previous observations, the bacteria most strongly associated with CRC belonged to taxa more commonly associated with the oral cavity, including Pophyromonas, Fusobacterium, Parvimonas, Peptostreptococcus,and Gemella [warren2013co,zeller2014potential] (`r canc_stripChart`). The ROC curve for the model had an AUC of 0.837, which is similar to AUCs reported for other microbiota-based models for CRC, but lower than that of FIT (`r signif(canc_fit_roc$auc, 3)`). This confirms that random forest model based on bacterial abundances can distinuish healhty individuals from those with cancer.
