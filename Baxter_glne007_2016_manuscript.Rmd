...
pdf('results/figures/Figure3.pdf', height=6, width=10)
...
legend(0,1, legend=c(sprintf('FIT (%.0fng/ml)',ceiling(fit_thr)),'MMT'), col=c('grey90','grey25'), pch=15, bty='n', pt.cex=2)
...
getMcnemarP <- function(model, fit){
  square <- matrix(c(length(model[model>=cutoff & fit >=fit_thr]),
                     length(model[model>=cutoff & fit<fit_thr]),
                     length(model[model<cutoff & fit >=fit_thr]),
                     length(model[model<cutoff & fit <fit_thr])), ncol=2, byrow=T)
  return(mcnemar.test(square)$p.value)
  }

if(getMcnemarP(all_les_probs, all_les_fit)<0.05){text(2,sens_matrix[2,1]+0.05, pval(getMcnemarP(all_les_probs, all_les_fit)),cex=0.8)}
if(getMcnemarP(nonAdv_probs, nonAdv_fit)<0.05){text(5,sens_matrix[2,2]+0.05, pval(getMcnemarP(nonAdv_probs, nonAdv_fit)),cex=0.8)}
if(getMcnemarP(advAde_probs, advAde_fit)<0.05){text(8,sens_matrix[2,3]+0.05, pval(getMcnemarP(advAde_probs, advAde_fit)),cex=0.8)}
if(getMcnemarP(all_ade_probs, all_ade_fit)<0.05){text(11,sens_matrix[2,4]+0.05, pval(getMcnemarP(all_ade_probs, all_ade_fit)),cex=0.8)}
if(getMcnemarP(s1_probs, s1_fit)<0.05){text(14,sens_matrix[2,5]+0.05, pval(getMcnemarP(s1_probs, s1_fit)),cex=0.8)}
if(getMcnemarP(s2_probs, s2_fit)<0.05){text(17,sens_matrix[2,6]+0.05, pval(getMcnemarP(s2_probs, s2_fit)),cex=0.8)}
if(getMcnemarP(s3_probs, s3_fit)<0.05){text(20,sens_matrix[2,7]+0.05, pval(getMcnemarP(s3_probs, s3_fit)),cex=0.8)}
if(getMcnemarP(s4_probs, s4_fit)<0.05){text(23,sens_matrix[2,8]+0.05, pval(getMcnemarP(s4_probs, s4_fit)),cex=0.8)}
if(getMcnemarP(all_canc_probs, all_canc_fit)<0.05){text(26,sens_matrix[2,1]+0.05, pval(getMcnemarP(all_canc_probs, all_canc_fit)),cex=0.8)}
