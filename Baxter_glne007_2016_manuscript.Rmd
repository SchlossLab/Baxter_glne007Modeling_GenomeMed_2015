...
```{r figure4_barplot, echo=F, fig.width=9, fig.height=6, results='hide'}
...
**Statistical Methods.**  All statistical analyses were performed using R (v.3.2.0). Random Forest models were generated using the AUCRF package [@calle2011auc]. The AUC of ROC curves were compared using the method described by DeLong et al. [@delong1988comparing]. The optimal cutoff for the MMT was determined using Youden's _J_ statistic as implemented in the pROC package in R [@youden1950index]. This cutoff was determined using the ROC curve for differentiating cancer from normal. The sensitivities of FIT and the MMT were compared using McNemar's Chi-squared test. To control for diagnosis while testing the effects of sex on the microbiome we used PERMANOVA as implemented in the adonis function in the vegan R package [@dixon2003vegan].
...
To make a fairer comparison of the sensitivities of these two tests, we reduced the cutoff for FIT to closely match the specificity of the MMT. At a 7 ng/ml cutoff FIT had a specificity of `r round(100*low_fit_spec,1)`%, compared to `r round(100*mmt_spec, 1)`% for the MMT. At the lower cutoff for FIT there was no significant difference in sensitivity for cancer between the two tests, but the MMT remained significantly more sensitive for detecting adenomas (p<0.001) and all lesions grouped together (p<0.001, `r bar`). 
...
**Figure 4. Sensitivities for FIT and MMT for each stage of tumor development with matching specificities.**  The cutoff for FIT was reduced to 7 ng/ml to closely match the specificity of the MMT. P-values based on McNemar's chi-squared test.
