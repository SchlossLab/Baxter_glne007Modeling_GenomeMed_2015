...
###Lesion vs. Normal
```{r lesion_ROC_FIT, warning=F, results='hold', echo=F, fig.width=7, fig.height=7, cache=T}
...
adenoma_fit_sens <- length(adenoma_fit[adenoma_fit>=1]) / length(adenoma_fit)
advAde_fit_sens <- length(advAde_fit[advAde_fit>=1]) / length(advAde_fit)
s1_fit_sens <- length(s1_fit[s1_fit>=1]) / length(s1_fit)
s2_fit_sens <- length(s2_fit[s2_fit>=1]) / length(s2_fit)
s3_fit_sens <- length(s3_fit[s3_fit>=1]) / length(s3_fit)
s4_fit_sens <- length(s4_fit[s4_fit>=1]) / length(s4_fit)
...
legend('topleft', legend=c('RF Model','FIT (1ng/ml cutoff)'), col=c('grey25','grey90'), pch=15, bty='n', pt.cex=2)


#McNemar test
getMcnemarP <- function(model, fit){
  square <- matrix(c(length(model[model>=cutoff & fit >=1]),
                     length(model[model>=cutoff & fit<1]),
                     length(model[model<cutoff & fit >=1]),
                     length(model[model<cutoff & fit <1])), ncol=2, byrow=T)
...
```{r male_vs_female, echo=F, fig.width=9, fig.height=6}
layout(matrix(c(1,2),nrow=1), widths=c(8,5))
par(mar=c(4.5,4.5,1,1))

m_roc <- roc(all.data$lesion[all.data$Gender == 'm'] ~ all_probs[all.data$Gender == 'm',2])
f_roc <- roc(all.data$lesion[all.data$Gender == 'f'] ~ all_probs[all.data$Gender == 'f',2])
...
stripchart(at=1, all_probs[all.data$dx=='normal' & all.data$Gender == 'm', 2], vertical=T, method='jitter', jitter=0.4, pch=21, bg='grey', add=T)
stripchart(at=2, all_probs[all.data$dx=='adenoma' & all.data$Gender == 'm',2], vertical=T, method='jitter', jitter=0.4, pch=21, bg='blue', add=T)
stripchart(at=3, all_probs[all.data$dx=='cancer' & all.data$Gender == 'm',2], vertical=T, method='jitter', jitter=0.4, pch=21, bg='red', add=T)

stripchart(at=4.5, all_probs[all.data$dx=='normal' & all.data$Gender == 'f',2], vertical=T, method='jitter', jitter=0.4, pch=21, bg='grey', add=T)
stripchart(at=5.5, all_probs[all.data$dx=='adenoma' & all.data$Gender == 'f',2], vertical=T, method='jitter', jitter=0.4, pch=21, bg='blue', add=T)
stripchart(at=6.6, all_probs[all.data$dx=='cancer' & all.data$Gender == 'f',2], vertical=T, method='jitter', jitter=0.4, pch=21, bg='red', add=T)
...
table(all_probs[all.data$dx=='normal' & all.data$Gender == 'm', 2] >= cutoff)
```


```{r nsaids, echo=F, fig.width=9, fig.height=6}
layout(matrix(c(1,2),nrow=1), widths=c(8,5))
par(mar=c(4.5,4.5,1,1))

nsaid_roc <- roc(all.data$lesion[all.data$NSAID == 1] ~ all_probs[all.data$NSAID == 1,2])
no_nsaid_roc <- roc(all.data$lesion[all.data$NSAID == 0] ~ all_probs[all.data$NSAID == 0,2])

nsaid_fit <- roc(all.data$lesion[all.data$NSAID == 1] ~ all.data$fit_result[all.data$NSAID == 1])
no_nsaid_fit <- roc(all.data$lesion[all.data$NSAID == 0] ~ all.data$fit_result[all.data$NSAID == 0])
...
plot(1, type="n", xlim=c(0.5,7), ylim=c(0,1), ylab="Model Probability of Lesion", xlab="", xaxt="n")
stripchart(at=1, all_probs[all.data$dx=='normal' & all.data$NSAID == 1, 2], vertical=T, method='jitter', jitter=0.4, pch=21, bg='grey', add=T)
stripchart(at=2, all_probs[all.data$dx=='adenoma' & all.data$NSAID == 1,2], vertical=T, method='jitter', jitter=0.4, pch=21, bg='blue', add=T)
stripchart(at=3, all_probs[all.data$dx=='cancer' & all.data$NSAID == 1,2], vertical=T, method='jitter', jitter=0.4, pch=21, bg='red', add=T)

stripchart(at=4.5, all_probs[all.data$dx=='normal' & all.data$NSAID == 0,2], vertical=T, method='jitter', jitter=0.4, pch=21, bg='grey', add=T)
stripchart(at=5.5, all_probs[all.data$dx=='adenoma' & all.data$NSAID == 0,2], vertical=T, method='jitter', jitter=0.4, pch=21, bg='blue', add=T)
stripchart(at=6.6, all_probs[all.data$dx=='cancer' & all.data$NSAID == 0,2], vertical=T, method='jitter', jitter=0.4, pch=21, bg='red', add=T)
