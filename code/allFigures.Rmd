...
```{r canc_OTUs, warning=F, echo=F, fig.width=7, fig.height=7, eval=FALSE}
...
```{r ade_OTUs, warning=F, echo=F, fig.width=7, fig.height=7, eval=FALSE}
taxonomy <- read.delim('data/glne007.final.an.unique_list.0.03.cons.taxonomy', sep='\t', header=T, row.names=1)
...
```{r combined_OTUs, warning=F, echo=F, fig.width=7, fig.height=9}
canc_otus <- canc_fit_model$Xopt
canc_otus <- canc_otus[canc_otus!='fit_result']
canc_otus <- rev(canc_otus)
...
c_norm_abunds <- all_data[all_data$dx=='normal', canc_otus]/10000 + 1e-4
canc_abunds <- all_data[all_data$dx=='cancer', canc_otus]/10000 + 1e-4

a_norm_abunds <- all_data[all_data$dx=='normal', ade_otus]/10000 + 1e-4
ade_abunds <- all_data[all_data$dx=='adenoma', ade_otus]/10000 + 1e-4


tax <- read.delim('data/glne007.0.03.trim.tax', sep='\t', header=T, row.names=1)
ade_tax <- paste(tax[ade_otus, 'Classification'], '\n(', tax[ade_otus, 'OTU'], ')', sep='')
canc_tax <- paste(tax[canc_otus, 'Classification'], '\n(', tax[canc_otus, 'OTU'], ')', sep='')

    
par(mar=c(5, 9, 0.5, 0.5))
plot(1, type="n", ylim=c(0,length(c(ade_otus,canc_otus))*2), xlim=c(1e-4,1), log="x", ylab="", xlab="Relative Abundance (%)", xaxt="n", yaxt="n")
index <- 1
for(i in ade_otus){
  stripchart(at=index-0.35, jitter(a_norm_abunds[,i], amount=1e-5), pch=21, bg="grey", method="jitter", jitter=0.2, add=T, cex=1)
  stripchart(at=index+0.35, jitter(ade_abunds[,i], amount=1e-5), pch=21, bg="blue", method="jitter", jitter=0.2, add=T, cex=1)
  segments(mean(a_norm_abunds[,i]),index-0.7,mean(a_norm_abunds[,i]),index, lwd=3)
  segments(mean(ade_abunds[,i]),index+0.7,mean(ade_abunds[,i]),index, lwd=3)
  index <- index + 2
}
for(i in canc_otus){
  stripchart(at=index-0.35, jitter(c_norm_abunds[,i], amount=1e-5), pch=21, bg="grey", method="jitter", jitter=0.2, add=T, cex=1)
  stripchart(at=index+0.35, jitter(canc_abunds[,i], amount=1e-5), pch=21, bg="red", method="jitter", jitter=0.2, add=T, cex=1)
  segments(mean(c_norm_abunds[,i]),index-0.7,mean(c_norm_abunds[,i]),index, lwd=3)
  segments(mean(canc_abunds[,i]),index+0.7,mean(canc_abunds[,i]),index, lwd=3)
  index <- index + 2
}

axis(2, at=seq(1,index-2,2), labels=c(ade_tax, canc_tax), las=1, tick=F, cex.axis=1)
axis(1, at=c(1e-4, 1e-3, 1e-2, 1e-1, 1), label=c("0", "0.1", "1", "10", "100"))
...
```{r les_OTUs, warning=F, echo=F, fig.width=7, fig.height=12}
