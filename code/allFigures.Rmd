...
```{r combined_OTUs, warning=F, echo=F, fig.width=7, fig.height=8}
canc_otus <- canc_fit_model$Xopt
canc_otus <- canc_otus[canc_otus!='fit_result']
canc_otus <- rev(canc_otus)
...
c_norm_abunds <- all_data[all_data$dx=='normal', canc_otus]/10547 + 1e-4
canc_abunds <- all_data[all_data$dx=='cancer', canc_otus]/10547 + 1e-4

a_norm_abunds <- all_data[all_data$dx=='normal', ade_otus]/10547 + 1e-4
ade_abunds <- all_data[all_data$dx=='adenoma', ade_otus]/10547 + 1e-4
...
index <- 1
for(i in ade_otus){
  stripchart(at=index-0.3, a_norm_abunds[,i], pch=21, bg="grey", method="jitter", jitter=0.2, add=T, cex=1)
  stripchart(at=index+0.3, ade_abunds[,i], pch=21, bg="blue", method="jitter", jitter=0.2, add=T, cex=1)
  index <- index + 2
}
for(i in canc_otus){
  stripchart(at=index-0.3, c_norm_abunds[,i], pch=21, bg="grey", method="jitter", jitter=0.2, add=T, cex=1)
  stripchart(at=index+0.3, canc_abunds[,i], pch=21, bg="red", method="jitter", jitter=0.2, add=T, cex=1)
  index <- index + 2
}

axis(2, at=seq(1,index-2,2), labels=c(ade_otus, canc_otus), las=1, tick=F, cex.axis=1)
axis(1, at=c(1e-4, 1e-3, 1e-2, 1e-1, 1), label=c("0", "0.1", "1", "10", "100"))
...
```{r les_OTUs, warning=F, echo=F, fig.width=7, fig.height=8}
taxonomy <- read.delim('data/glne007.final.an.unique_list.0.03.cons.taxonomy', sep='\t', header=T, row.names=1)
...
index <- 1
for(i in les_otus){
  stripchart(at=index-0.3, norm_abunds[,i], pch=21, bg="grey", method="jitter", jitter=0.2, add=T, cex=1)
  stripchart(at=index+0.3, les_abunds[,i], pch=21, bg="green4", method="jitter", jitter=0.2, add=T, cex=1)
  index <- index + 2
}
